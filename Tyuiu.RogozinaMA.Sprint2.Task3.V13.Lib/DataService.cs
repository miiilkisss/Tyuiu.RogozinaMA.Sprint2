    ...
    public class DataService : ISprint2Task3V13
    {
        public double Calculate(double x)
        {
            ...
            if (x > 1)
            {
                ...
                double numerator = x - Math.Pow((x + 1) / (x - 1), x);
            ...
            {
                // Для x = 1, x = 2, x = -17 - неопределенные случаи
                throw new ArgumentException("Значение x не попадает в определенные интервалы");
            }
